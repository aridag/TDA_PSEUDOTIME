...
#==========
Labvalues <-
  as.data.frame(apply(FupData[, names(FupData) %in% LabvaluesNames], 2, as.numeric))
...
colfunc <-
  colorRampPalette(c("#00A3DD", "#60C659", "#FFBC21", "#FF7F1E", "#EF2B2D"))
...
clrlookup <-
  data.frame(clrs = colfunc(length(unique(
    y.mean.vertex$value
  ))),
  value = sort(unique(y.mean.vertex$value)))

clrMap <- merge(y.mean.vertex,
                clrlookup,
                by = c("value"),
                all.x = TRUE)
...
V(F2.graph)$size <-
  (((vertex.size - min(vertex.size)) / (max(vertex.size) - min(vertex.size))) *
     6) + 8
plot(F2.graph)
...
for (j in 1:length(E(F2.graph))) {
  ...
  # ========  TIME WEIGHT
  E(F2.graph)$weight[j] <-
    mean(f.time[f.time$ID %in%  rownames((Dist.Mtrx))[commonIDS], c("val")])
...
CommunityCluster <- edge.betweenness.community(
  F2.graph,
...
for (i in 1:F2$num_vertices) {
  points.in.vertex <- F2$points_in_vertex[[i]]
  FupData[FupData$id %in%  rownames((Dist.Mtrx))[points.in.vertex], c("node")] <-
    paste(i)
}
...
FupData <- merge(FupData, NodeClustes, by.x = 'node', by.y = 'Node')
...
#Plot Time
ggplot(FupData[!is.na(FupData$Cluster), ],  aes(
  x = as.factor(Cluster),
  y = as.numeric(time),
  fill = as.factor(Cluster)
)) +
  geom_boxplot(alpha = 0.8) + scale_fill_manual(values = Palette$Color) + scale_color_manual(values =
                                                                                               Palette$Color) +
...
pltList <- lapply(LabvaluesNames, function(i) {
  ggplot(FupData[is.na(FupData$Cluster) == FALSE, ], aes(
    x = as.factor(Cluster),
    y = as.numeric(FupData[is.na(FupData$Cluster) == FALSE, i]),
    fill = as.factor(Cluster)
  )) +
    geom_boxplot(alpha = 0.8) + ggtitle(i) + ylab("") + xlab("") +  scale_fill_manual(values =
                                                                                        Palette$Color) + scale_color_manual(values = Palette$Color) +
    theme(legend.position = "none",
          plot.title = element_text(size = 8, hjust = 0.5)) +
    scale_y_continuous(limits = quantile(as.numeric(na.omit(FupData[is.na(FupData$Cluster) ==
                                                                      FALSE, i])), c(0.1, 0.9)))
...
#========
StartingNodes <-
  V(minspantreeweights)[betweenness(minspantreeweights) == min(betweenness(minspantreeweights))]
EndingNodes <-
  V(minspantreeweights)[betweenness(minspantreeweights) == max(betweenness(minspantreeweights))]
  ...
  for (t in 1:length(Trajectories)) {
    if (length(Trajectories[[t]]$res) > 0) {
      temp = data.frame(
        ...
        JW =  stringsim(
          paste(Trajectories[[t]]$res[[1]], collapse = "_"),
...
pltList <- lapply(LabvaluesNames, function(i) {
  ggplot(FupDataTraj, aes(time, as.numeric(FupDataTraj[, i]), colour = as.factor(trajNumbManual))) +
    geom_smooth(aes(
      group = as.factor(trajNumbManual),
      fill = as.factor(trajNumbManual),
      method = "loess",
      se = FALSE
    )) +
    ggtitle(i) + ylab("") + xlab("") +  theme(legend.position = "none",
                                              plot.title = element_text(size = 8, hjust = 0.5))
